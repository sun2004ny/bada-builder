...
const ExhibitionIndividualHero = ({ title, subtitle }) => {
  ...
  // Split title into two specific lines: "Properties by Individual" and "Owners"
  const titleLines = ["Properties by Individual", "Owners"];
      ...
      <div className="hero-glass-box">
        <div className="hero-content-flex">
          <div className="hero-left">
            ...
            >
              <span className="badge-text">Verified Listings</span>
...
const ByIndividual = () => {
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await propertiesAPI.getAll({ user_type: "individual", status: "active" });
  ...
  return (
    <FloatingBackground>
      <ExhibitionIndividualHero
        title="Properties by Individual Owners"
        subtitle="Direct listings from property owners - No middleman, better deals. Experience a premium, modern marketplace."
      />
        ...
        <div className="content-grid-area">
            ...
            {!loading && !error && (
              ...
              >
                {properties.map((property, index) => (
                  ...
                  >
                    <PropertyCard
                      property={{ ...property, image: property.image_url, area: property.area || property.size, status: property.status || "Available", badge: "Individual", owner: property.owner_name || "Property Owner" }}
                      viewType={view}
                      source="individual"
                    />
                  </motion.div>
          ...
          {!loading && !error && properties.length === 0 && (
            <motion.div className="empty-state" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <Link to="/post-property" className="post-property-link">Be the first to post a property!</Link>
            </motion.div>
          )}
        </div>
      </div>
      ...
      <style jsx>{`
        ...
        .hero-glass-box { 
          width: calc(100% - 48px); /* px-6 side space */
          max-width: 1400px; 
        ...
        .floating-property-icons { 
          position: absolute; 
          top: 55%; 
          right: -40px; /* Moved further right for mobile view */
        ...
        @media (min-width: 1024px) {
          .floating-property-icons {
            top: 20px; /* Specific pixel value for top to move it up */
            right: 40px;
            transform: scale(1.0); /* Removed translateY centering to allow top to work */
            width: 500px;
            height: 500px;
            opacity: 0.75;
          }
        ...
        .animated-tabs-container { display: flex; justify-content: center; margin-bottom: 40px; position: relative; z-index: 10; }
        .tabs-glass-wrapper { display: flex; padding: 8px; background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(20px); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 20px; gap: 8px; }
        .animated-tab { position: relative; padding: 12px 24px; text-decoration: none; border-radius: 12px; color: rgba(255, 255, 255, 0.6); font-weight: 600; display: flex; align-items: center; }
        .animated-tab.active { color: white; }
        .active-pill-bg { position: absolute; inset: 0; background: linear-gradient(135deg, #7c3aed, #4f46e5); border-radius: 12px; z-index: 1; }
        .active-glow { position: absolute; bottom: -4px; left: 50%; transform: translateX(-50%); width: 40%; height: 2px; background: #a78bfa; filter: blur(4px); z-index: 1; }
        ...
        .post-property-link { display: inline-block; margin-top: 20px; padding: 12px 30px; background: linear-gradient(135deg, #7c3aed, #4f46e5); color: white; border-radius: 12px; text-decoration: none; font-weight: 700; box-shadow: 0 4px 15px rgba(124, 58, 237, 0.4); transition: all 0.3s ease; }
        .post-property-link:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(124, 58, 237, 0.5); }
      `}</style>
    </FloatingBackground>
  );
};
