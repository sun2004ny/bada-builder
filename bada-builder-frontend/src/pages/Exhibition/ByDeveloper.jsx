...
const ExhibitionDeveloperHero = ({ title, subtitle }) => {
  ...
  // Split title into two specific lines: "Properties by" and "Developers"
  const titleLines = ["Properties by", "Developers"];
      ...
      <div className="hero-glass-box">
        <div className="hero-content-flex">
          <div className="hero-left">
            ...
            >
              <span className="badge-text">Verified Projects</span>
...
const ByDeveloper = () => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await propertiesAPI.getAll({ user_type: "developer", status: "active" });
        const projectsData = response.properties || response || [];
        const activeProjects = await filterAndMarkExpiredProperties(projectsData);
        activeProjects.sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0));
  ...
  return (
    <FloatingBackground>
      <ExhibitionDeveloperHero
        title="Properties by Developers"
        subtitle="Explore premium projects directly from trusted developers. Verified listings, better pricing, and modern living options."
      />
